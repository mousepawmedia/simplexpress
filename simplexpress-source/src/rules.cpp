...
{
	// determines whether or not should be upper or lower case. If unspecified
	// then will be either case
	switch (letterCase) {
...
bool Rule::rule_o(onechar ch)
{
	...
	/*switch statement to determine what to return. This allows for user
	addition of more operators*/
	switch (*ch.c_str()) {
		case '+':
		case '-':
		case '*':
...
// Carriage Return
bool Rule::rule_r(onechar ch) { return ch == '\r'; }

// Literal Space
bool Rule::rule_s(onechar ch) { return ch == ' '; }

// Tab
bool Rule::rule_t(onechar ch) { return ch == '\t'; }
...
// Whitespace
bool Rule::rule_w(onechar ch) { return Specifier::s_whitespace(ch); }
